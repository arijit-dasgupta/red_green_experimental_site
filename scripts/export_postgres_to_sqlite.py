...
def main():
    ...
    # SQLite schema matching the app's current models.
    # We intentionally skip foreign keys for compatibility with DB Browser and older SQLite builds.
    ddl = """
CREATE TABLE IF NOT EXISTS redgreen_session (
    ...
    post_experiment_feedback_submitted INTEGER NOT NULL DEFAULT 0
);
    ...
    if not args.all:
        try:
            ...
            if "column" in str(e).lower() and "does not exist" in str(e).lower():
                print(
                    "Warning: dataset_name/experiment_run_version columns missing (old DB); exporting all sessions.",
                    file=sys.stderr,
                )
            else:
                raise
