...
const P20CanvasPage = ({ fetchNextScene, setdisableCountdownTrigger }) => {
    ...
    useEffect(() => {
        if (config.startingAudioPath && config.startingAudioPath.trim() !== '') {
            ...
            audio.onloadeddata = () => {
                ...
                console.log('P20CanvasPage: Starting audio file loaded:', config.startingAudioPath);
            };
            audio.onerror = () => {
                console.warn('P20CanvasPage: Failed to load starting audio file from:', config.startingAudioPath);
    ...
    useEffect(() => {
        if (config.endingAudioPath && config.endingAudioPath.trim() !== '') {
            ...
            audio.onloadeddata = () => {
                ...
                console.log('P20CanvasPage: Ending audio file loaded:', config.endingAudioPath);
            };
            audio.onerror = () => {
                console.warn('P20CanvasPage: Failed to load ending audio file from:', config.endingAudioPath);
        ...
        const loadTrialData = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log('✅ P20CanvasPage: Successfully loaded trial data');
                    console.log('📊 P20CanvasPage: Data summary:', {
    ...
    const renderFrame = React.useCallback((frameIndex) => {
        ...
        if (!canvas || !sceneData) {
            console.log('P20CanvasPage: renderFrame skipped - canvas or sceneData missing', { canvas: !!canvas, sceneData: !!sceneData });
            return;
        }
        ...
        if (step_data && step_data[frameIndex]) {
            ...
            // Debug logging
            if (frameIndex === 0) {
                console.log('P20CanvasPage: Rendering ball at frame 0', {
                ...
                if (nextFrame < maxFrames) {
                    ...
                    if (endAudioRef.current) {
                        ...
                        endAudioRef.current.play().catch(error => {
                            console.warn('P20CanvasPage: Failed to play end audio:', error);
                        });
                        console.log('🔊 P20CanvasPage: End sound played');
                    }
                    if (startTimeRef.current) {
                        const actualDuration = (timestamp - startTimeRef.current) / 1000;
                        const expectedDuration = maxFrames / (sceneData.fps || 30);
                        console.log(`🏁 P20CanvasPage: Video finished - Frame ${maxFrames - 1}/${maxFrames}`);
                        console.log(`⏱️ P20CanvasPage: Expected duration: ${expectedDuration.toFixed(2)}s, Actual duration: ${actualDuration.toFixed(2)}s`);
                        console.log(`📊 P20CanvasPage: Recorded ${recordedKeyStates.current.length} key state frames`);
                    }
    ...
    useEffect(() => {
        ...
        if (isNewSceneData) {
            ...
            console.log('P20CanvasPage: Scene data loaded, starting countdown automatically');
            console.log(`📊 P20CanvasPage: Video info - Frames: ${maxFrames}, FPS: ${fps}, Expected duration: ${expectedVideoDuration.toFixed(2)}s`);
            setIsPlaying(false);
            setCurrentFrame(0);
            ...
            countdownIntervalRef.current = setInterval(() => {
                ...
                if (countdownValue === 0) {
                    ...
                    // Start video animation after countdown
                    startTimeRef.current = performance.now();
                    console.log('🎬 P20CanvasPage: Starting video animation at', new Date().toISOString());
                    setIsPlaying(true);
    ...
    useEffect(() => {
        console.log("🔍 P20CanvasPage: Auto-advance useEffect running", { 
            showCongratulations, 
        ...
        if (showCongratulations && !congratulationsTimerRef.current && !hasAutoAdvancedRef.current) {
            console.log("⏱️ P20CanvasPage: Setting up auto-advance timer (2s delay)...");
            ...
            congratulationsTimerRef.current = setTimeout(() => {
                console.log("🎬 P20CanvasPage: Timer fired! Auto-advancing to next page...");
                ...
                // Call fetchNextScene
                console.log("🎬 P20CanvasPage: Calling fetchNextScene...");
                if (fetchNextScene) {
                    try {
                        ...
                        if (result && typeof result.then === 'function') {
                            result.then(() => {
                                console.log("✅ P20CanvasPage: fetchNextScene completed");
                            }).catch((error) => {
                                console.error("❌ P20CanvasPage: fetchNextScene error:", error);
                            });
                        } else {
                            console.log("✅ P20CanvasPage: fetchNextScene called (non-promise return)");
                        }
                    } catch (error) {
                        console.error("❌ P20CanvasPage: Error calling fetchNextScene:", error);
                    }
                } else {
                    console.error("❌ P20CanvasPage: fetchNextScene is not defined!");
                }
            }, 2000); // 2 second delay after congratulations page
            
            console.log("⏱️ P20CanvasPage: Timer set with ID:", congratulationsTimerRef.current);
        }
        ...
        return () => {
            ...
            if (congratulationsTimerRef.current && !showCongratulations) {
                console.log("🧹 P20CanvasPage: Cleaning up auto-advance timer (showCongratulations became false)");
                clearTimeout(congratulationsTimerRef.current);
                congratulationsTimerRef.current = null;
    ...
    useEffect(() => {
        const handleKeyPress = (e) => {
            if (e.shiftKey && (e.key === 'S' || e.key === 's')) {
                console.log("SKIP KEY PRESSED: Shift+S detected in P20CanvasPage, skipping to next page");
                e.preventDefault();
                e.stopPropagation();
    ...
    return (
        <div style={{
            ...
            {showCongratulations && (() => {
                console.log("🎉 P20CanvasPage: Rendering congratulations page!");
                return (
                <div style={{
                    position: "absolute",
                    top: 0,
