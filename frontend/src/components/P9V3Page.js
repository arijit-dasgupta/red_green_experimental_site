...
/**
 * P9V3Page - Key Switching Practice 1 (Interactive)
 * 
 * Canvas: T_v3_keyswitch_practice_1 trial data
 * No audio
 * Behavior: 3-2-1 countdown, interactive canvas with key tracking, congratulations, auto-advance
 */
const P9V3Page = ({ onComplete }) => {
    const { isPaused, resumeCounter } = usePause();
    
    // Only log on actual mount, not every render
    useEffect(() => {
        console.log("🎬 P9V3Page: Component MOUNTED");
        return () => console.log("🎬 P9V3Page: Component UNMOUNTED");
    }, []);
    ...
    useEffect(() => {
        const loadTrialData = async () => {
            try {
                console.log('📥 P9V3Page: Loading T_v3_keyswitch_practice_1 trial data...');
                const response = await fetch('/api/load_trial_data/T_v3_keyswitch_practice_1', {
                    method: 'GET',
                ...
                if (response.ok) {
                    ...
                    console.log('✅ P9V3Page: Successfully loaded trial data');
                    setSceneData(data);
                } else {
                    console.error('❌ P9V3Page: Failed to load trial data:', response.status);
                }
            } catch (error) {
                console.error('❌ P9V3Page: Error loading trial data:', error);
            }
        };
        loadTrialData();
    }, []);
                ...
                if (nextFrame < maxFrames) {
                    ...
                    if (endAudioRef.current) {
                        ...
                        endAudioRef.current.play().catch(error => {
                            console.warn('P9V3Page: Failed to play end audio:', error);
                        });
                    }
                }
            } else {
    ...
    useEffect(() => {
        if (resumeCounter > 0 && resumeCounter !== lastResumeCounterRef.current) {
            ...
            console.log('▶️ P9V3Page: Resumed - restarting from beginning');
    ...
    const startCountdown = useCallback(() => {
        ...
        countdownIntervalRef.current = setInterval(() => {
            ...
            if (countdownValue === 0) {
                ...
                setCountdown(null);
                console.log('🎬 P9V3Page: Starting video animation');
                setIsPlaying(true);
                ...
                if (startAudioRef.current) {
                    ...
                    startAudioRef.current.play().catch(error => {
                        console.warn('P9V3Page: Failed to play start audio:', error);
                    });
                }
            }
        }, 750);
    ...
    useEffect(() => {
        ...
        if (isNewSceneData) {
            ...
            console.log('P9V3Page: Scene data loaded, starting countdown');
            setIsPlaying(false);
            setCurrentFrame(0);
            currentFrameRef.current = 0;
            setVideoFinished(false);
    ...
    useEffect(() => {
        if (videoFinished && !showCongratulations) {
            console.log("🎉 P9V3Page: Video finished, showing congratulations");
    ...
    useEffect(() => {
        if (showCongratulations && !congratulationsTimerRef.current && !hasAutoAdvancedRef.current) {
            ...
            congratulationsTimerRef.current = setTimeout(() => {
                console.log("🎬 P9V3Page: Auto-advancing to next page");
                congratulationsTimerRef.current = null;
    ...
    useEffect(() => {
        const handleKeyPress = (e) => {
            if (e.shiftKey && (e.key === 'S' || e.key === 's')) {
                console.log("⏭️ P9V3Page: Skip pressed");
                e.preventDefault();
                if (onCompleteRef.current) onCompleteRef.current();
            }
        };
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                P9V3 | F: {keyStates.f.toString()} | J: {keyStates.j.toString()}
            </div>
        </div>
    );
};

export default P9V3Page;
