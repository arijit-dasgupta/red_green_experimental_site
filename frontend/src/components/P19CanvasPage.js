...
/**
 * Dedicated component for p19: Interactive canvas page (practice trial)
 * - Canvas: T_v2_keyswitch_ball_stable trial data (V2)
 ...
 */
const P19CanvasPage = ({ fetchNextScene, setdisableCountdownTrigger }) => {
    console.log("🎬 P19CanvasPage: Component mounted/rendered");
    ...
    // Start and end audio disabled for P14 (V2 design)
    // The startAudioRef and endAudioRef remain null - no start/end sounds
    useEffect(() => {
        console.log('P19CanvasPage: Start and end sounds disabled for V2');
    }, []);

    // V2: Load narration audio file (v2_keyswitch_ball_stable.mp3)
    useEffect(() => {
        const audio = new Audio('/audios/v2_keyswitch_ball_stable.mp3');
        audio.preload = 'auto';
        audio.onloadeddata = () => {
            narrationAudioRef.current = audio;
            console.log('P19CanvasPage: Narration audio file loaded: v2_keyswitch_ball_stable.mp3');
        };
        audio.onerror = () => {
            console.warn('P19CanvasPage: Failed to load narration audio file');
        ...
        audio.onended = () => {
            console.log('🏁 P19CanvasPage: Narration audio ended');
            setNarrationAudioFinished(true);
        };
    }, []);
    ...
    // Load T_v2_keyswitch_ball_stable trial data (V2)
    useEffect(() => {
        const loadTrialData = async () => {
            try {
                console.log('📥 P19CanvasPage: Loading T_v2_keyswitch_ball_stable trial data from /api/load_trial_data/T_v2_keyswitch_ball_stable...');
                const response = await fetch('/api/load_trial_data/T_v2_keyswitch_ball_stable', {
                    method: 'GET',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    console.log('✅ P19CanvasPage: Successfully loaded trial data');
                    console.log('📊 P19CanvasPage: Data summary:', {
                    ...
                    console.error('❌ P19CanvasPage: Failed to load T_v2_keyswitch_ball_stable data:', response.status, errorText);
                }
            } catch (error) {
                console.error('❌ P19CanvasPage: Error loading trial data:', error);
            }
        };
    ...
    const renderFrame = React.useCallback((frameIndex) => {
        ...
        if (!canvas || !sceneData) {
            console.log('P19CanvasPage: renderFrame skipped - canvas or sceneData missing', { canvas: !!canvas, sceneData: !!sceneData });
            return;
        }
        ...
        if (step_data && step_data[frameIndex]) {
            ...
            // Debug logging
            if (frameIndex === 0) {
                console.log('P19CanvasPage: Rendering ball at frame 0', {
                ...
                if (nextFrame < maxFrames) {
                    ...
                    if (endAudioRef.current) {
                        ...
                        endAudioRef.current.play().catch(error => {
                            console.warn('P19CanvasPage: Failed to play end audio:', error);
                        });
                        console.log('🔊 P19CanvasPage: End sound played');
                    }
                    if (startTimeRef.current) {
                        const actualDuration = (timestamp - startTimeRef.current) / 1000;
                        const expectedDuration = maxFrames / (sceneData.fps || 30);
                        console.log(`🏁 P19CanvasPage: Video finished - Frame ${maxFrames - 1}/${maxFrames}`);
                        console.log(`⏱️ P19CanvasPage: Expected duration: ${expectedDuration.toFixed(2)}s, Actual duration: ${actualDuration.toFixed(2)}s`);
                        console.log(`📊 P19CanvasPage: Recorded ${recordedKeyStates.current.length} key state frames`);
                    }
    ...
    useEffect(() => {
        if (isPaused) {
            console.log('⏸️ P19CanvasPage: Study paused - stopping animation and timers');
            // Stop animation
            if (animationRef.current) {
                cancelAnimationFrame(animationRef.current);
                animationRef.current = null;
    ...
    useEffect(() => {
        ...
        if (resumeCounter > 0 && resumeCounter !== lastResumeCounterRef.current) {
            ...
            console.log('▶️ P19CanvasPage: Study resumed - resetting and restarting from beginning');
                ...
                setTimeout(() => {
                    ...
                    countdownIntervalRef.current = setInterval(() => {
                        ...
                        if (countdownValue === 0) {
                            ...
                            console.log('🎬 P19CanvasPage: Restarting video animation after resume');
                            setIsPlaying(true);
                            ...
                            if (startAudioRef.current) {
                                ...
                                startAudioRef.current.play().catch(error => {
                                    console.warn('P19CanvasPage: Failed to play start audio:', error);
                                });
                            }
                            ...
                            if (narrationAudioRef.current) {
                                ...
                                narrationAudioRef.current.play().catch(error => {
                                    console.warn('P19CanvasPage: Failed to play narration audio:', error);
                                });
                            }
                        }
                    }, 750);
    ...
    useEffect(() => {
        ...
        if (isNewSceneData) {
            ...
            console.log('P19CanvasPage: Scene data loaded, starting countdown automatically');
            console.log(`📊 P19CanvasPage: Video info - Frames: ${maxFrames}, FPS: ${fps}, Expected duration: ${expectedVideoDuration.toFixed(2)}s`);
            setIsPlaying(false);
            setCurrentFrame(0);
            ...
            countdownIntervalRef.current = setInterval(() => {
                ...
                if (countdownValue === 0) {
                    ...
                    // Start video animation after countdown
                    startTimeRef.current = performance.now();
                    console.log('🎬 P19CanvasPage: Starting video animation at', new Date().toISOString());
                    setIsPlaying(true);
                    ...
                    if (startAudioRef.current) {
                        ...
                        console.log('🔊 P19CanvasPage: Start sound played');
                    }
                    
                    // V2: Play narration audio (v2_keyswitch_ball_stable.mp3)
                    if (narrationAudioRef.current) {
                        narrationAudioRef.current.currentTime = 0;
                        narrationAudioRef.current.play().catch(error => {
                            console.warn('P19CanvasPage: Failed to play narration audio:', error);
                        });
                        console.log('🔊 P19CanvasPage: Narration audio (v2_keyswitch_ball_stable.mp3) started');
                    }
                }
    ...
    useEffect(() => {
        console.log("🔍 P19CanvasPage: Checking finished states:", { videoFinished, narrationAudioFinished, showCongratulations });
        if (videoFinished && narrationAudioFinished && !showCongratulations) {
            console.log("🎉 P19CanvasPage: Both video and narration audio finished, showing congratulations page...");
            setShowCongratulations(true);
        } else if (videoFinished && narrationAudioFinished && showCongratulations) {
            console.log("✅ P19CanvasPage: Both finished and congratulations already showing");
        } else if (!videoFinished || !narrationAudioFinished) {
            console.log("⏳ P19CanvasPage: Video not finished yet");
        }
    }, [videoFinished, narrationAudioFinished, showCongratulations]);

    // Auto-advance 2s after congratulations page is shown
    useEffect(() => {
        console.log("🔍 P19CanvasPage: Auto-advance useEffect running", { 
            showCongratulations, 
        ...
        if (showCongratulations && !congratulationsTimerRef.current && !hasAutoAdvancedRef.current) {
            console.log("⏱️ P19CanvasPage: Setting up auto-advance timer (2s delay)...");
            ...
            congratulationsTimerRef.current = setTimeout(() => {
                console.log("🎬 P19CanvasPage: Timer fired! Auto-advancing to next page...");
                ...
                // Call fetchNextScene
                console.log("🎬 P19CanvasPage: Calling fetchNextScene...");
                if (fetchNextScene) {
                    try {
                        ...
                        if (result && typeof result.then === 'function') {
                            result.then(() => {
                                console.log("✅ P19CanvasPage: fetchNextScene completed");
                            }).catch((error) => {
                                console.error("❌ P19CanvasPage: fetchNextScene error:", error);
                            });
                        } else {
                            console.log("✅ P19CanvasPage: fetchNextScene called (non-promise return)");
                        }
                    } catch (error) {
                        console.error("❌ P19CanvasPage: Error calling fetchNextScene:", error);
                    }
                } else {
                    console.error("❌ P19CanvasPage: fetchNextScene is not defined!");
                }
            }, 2000); // 2 second delay after congratulations page
            
            console.log("⏱️ P19CanvasPage: Timer set with ID:", congratulationsTimerRef.current);
        }
        ...
        return () => {
            ...
            if (congratulationsTimerRef.current && !showCongratulations) {
                console.log("🧹 P19CanvasPage: Cleaning up auto-advance timer (showCongratulations became false)");
                clearTimeout(congratulationsTimerRef.current);
                congratulationsTimerRef.current = null;
    ...
    useEffect(() => {
        const handleKeyPress = (e) => {
            if (e.shiftKey && (e.key === 'S' || e.key === 's')) {
                console.log("SKIP KEY PRESSED: Shift+S detected in P19CanvasPage, skipping to next page");
                e.preventDefault();
                e.stopPropagation();
    ...
    return (
        <div style={{
            ...
            {showCongratulations && (() => {
                console.log("🎉 P19CanvasPage: Rendering congratulations page!");
                return (
                <div style={{
                    position: "absolute",
                    top: 0,
