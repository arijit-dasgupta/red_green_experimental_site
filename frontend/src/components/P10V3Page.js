...
/**
 * P10V3Page - Key Switching Practice 2 (Interactive)
 * 
 * Canvas: T_v3_keyswitch_practice_2 trial data
 * No audio
...
const P10V3Page = ({ onComplete }) => {
    ...
    useEffect(() => {
        console.log("🎬 P10V3Page: Component MOUNTED");
        return () => console.log("🎬 P10V3Page: Component UNMOUNTED");
    }, []);
    ...
    useEffect(() => {
        const loadTrialData = async () => {
            try {
                console.log('📥 P10V3Page: Loading T_v3_keyswitch_practice_2 trial data...');
                const response = await fetch('/api/load_trial_data/T_v3_keyswitch_practice_2', {
                    method: 'GET',
                ...
                if (response.ok) {
                    ...
                    console.log('✅ P10V3Page: Successfully loaded trial data');
                    setSceneData(data);
                } else {
                    console.error('❌ P10V3Page: Failed to load trial data:', response.status);
                }
            } catch (error) {
                console.error('❌ P10V3Page: Error loading trial data:', error);
            }
        };
        loadTrialData();
    }, []);
                ...
                if (nextFrame < maxFrames) {
                    ...
                    if (endAudioRef.current) {
                        ...
                        endAudioRef.current.play().catch(error => {
                            console.warn('P10V3Page: Failed to play end audio:', error);
                        });
                    }
                }
            } else {
    ...
    useEffect(() => {
        if (isPaused) {
            console.log('⏸️ P10V3Page: Paused');
            if (animationRef.current) {
                cancelAnimationFrame(animationRef.current);
                animationRef.current = null;
            }
    ...
    useEffect(() => {
        if (resumeCounter > 0 && resumeCounter !== lastResumeCounterRef.current) {
            ...
            console.log('▶️ P10V3Page: Resumed - restarting from beginning');
    ...
    const startCountdown = useCallback(() => {
        ...
        countdownIntervalRef.current = setInterval(() => {
            ...
            if (countdownValue === 0) {
                ...
                setCountdown(null);
                console.log('🎬 P10V3Page: Starting video animation');
                setIsPlaying(true);
                ...
                if (startAudioRef.current) {
                    ...
                    startAudioRef.current.play().catch(error => {
                        console.warn('P10V3Page: Failed to play start audio:', error);
                    });
                }
            }
        }, 750);
    ...
    useEffect(() => {
        ...
        if (isNewSceneData) {
            ...
            console.log('P10V3Page: Scene data loaded, starting countdown');
            setIsPlaying(false);
            setCurrentFrame(0);
            currentFrameRef.current = 0;
            setVideoFinished(false);
    ...
    useEffect(() => {
        if (showCongratulations && !congratulationsTimerRef.current && !hasAutoAdvancedRef.current) {
            ...
            congratulationsTimerRef.current = setTimeout(() => {
                console.log("🎬 P10V3Page: Auto-advancing to next page");
                congratulationsTimerRef.current = null;
    ...
    useEffect(() => {
        const handleKeyPress = (e) => {
            if (e.shiftKey && (e.key === 'S' || e.key === 's')) {
                console.log("⏭️ P10V3Page: Skip pressed");
                e.preventDefault();
                if (onCompleteRef.current) onCompleteRef.current();
            }
        };
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                P10V3 | F: {keyStates.f.toString()} | J: {keyStates.j.toString()}
            </div>
        </div>
    );
};

export default P10V3Page;
