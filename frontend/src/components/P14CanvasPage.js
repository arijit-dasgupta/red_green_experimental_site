    ...
    useEffect(() => {
        if (config.startingAudioPath && config.startingAudioPath.trim() !== '') {
            ...
            audio.onloadeddata = () => {
                ...
                console.log('P14CanvasPage: Starting audio file loaded:', config.startingAudioPath);
            };
            audio.onerror = () => {
                console.warn('P14CanvasPage: Failed to load starting audio file from:', config.startingAudioPath);
    ...
    useEffect(() => {
        if (config.endingAudioPath && config.endingAudioPath.trim() !== '') {
            ...
            audio.onloadeddata = () => {
                ...
                console.log('P14CanvasPage: Ending audio file loaded:', config.endingAudioPath);
            };
            audio.onerror = () => {
                console.warn('P14CanvasPage: Failed to load ending audio file from:', config.endingAudioPath);
                ...
                if (nextFrame < maxFrames) {
                    ...
                    if (endAudioRef.current) {
                        ...
                        endAudioRef.current.play().catch(error => {
                            console.warn('P14CanvasPage: Failed to play end audio:', error);
                        });
                        console.log('🔊 P14CanvasPage: End sound played');
                    }
            ...
            countdownIntervalRef.current = setInterval(() => {
                ...
                if (countdownValue === 0) {
                    ...
                    if (startAudioRef.current) {
                        ...
                        console.log('🔊 P14CanvasPage: Start sound played');
                    }
                }
