...
const P21CanvasPage = ({ fetchNextScene, setdisableCountdownTrigger }) => {
    ...
    useEffect(() => {
        ...
        const loadTrialData = async () => {
            try {
                ...
                console.log('🔍 P21CanvasPage: VERIFY - API URL =', apiUrl);
                console.log('🔍 P21CanvasPage: FULL API URL =', window.location.origin + apiUrl);
    ...
    const renderFrame = React.useCallback((frameIndex) => {
        ...
        if (!canvas || !sceneData) {
            console.log('P21CanvasPage: renderFrame skipped - canvas or sceneData missing', { canvas: !!canvas, sceneData: !!sceneData });
            return;
        }
        ...
        if (step_data && step_data[frameIndex]) {
            ...
            // Debug logging
            if (frameIndex === 0) {
                console.log('P21CanvasPage: Rendering ball at frame 0', {
            ...
            if (framesToAdvance > 0) {
                ...
                if (nextFrame < maxFrames) {
                    ...
                    if (startTimeRef.current) {
                        ...
                        console.log(`🏁 P21CanvasPage: Video finished - Frame ${maxFrames - 1}/${maxFrames}`);
                        console.log(`⏱️ P21CanvasPage: Expected duration: ${expectedDuration.toFixed(2)}s, Actual duration: ${actualDuration.toFixed(2)}s`);
                    }
    ...
    useEffect(() => {
        if (sceneData) {
            ...
            console.log('P21CanvasPage: Scene data loaded, starting animation and audio');
            console.log(`📊 P21CanvasPage: Video info - Frames: ${maxFrames}, FPS: ${fps}, Expected duration: ${expectedVideoDuration.toFixed(2)}s`);
            setIsPlaying(false);
            setCurrentFrame(0);
            ...
            setTimeout(() => {
                ...
                console.log('🎬 P21CanvasPage: Starting video animation at', new Date().toISOString());
                setIsPlaying(true);
            }, 100);
            
            // Start audio playback
            if (audioRef.current) {
                console.log('🔊 P21CanvasPage: Starting audio playback');
                console.log('🔊 P21CanvasPage: Audio src:', audioRef.current.src);
                console.log('🔊 P21CanvasPage: Audio readyState:', audioRef.current.readyState);
                audioRef.current.currentTime = 0; // Reset audio to start
                const playPromise = audioRef.current.play();
                if (playPromise !== undefined) {
                    playPromise
                        .then(() => {
                            console.log('✅ P21CanvasPage: Audio playback started successfully');
                            console.log('🔊 P21CanvasPage: Audio duration:', audioRef.current.duration, 'seconds');
                            console.log(`📊 P21CanvasPage: Audio should finish at ${audioRef.current.duration.toFixed(2)}s`);
                        })
                        .catch(error => {
                            console.error("❌ P21CanvasPage: Audio autoplay prevented:", error);
                        });
                }
            } else {
                console.error('❌ P21CanvasPage: Audio ref is null - cannot play audio');
            }
        }
    }, [sceneData]);
    ...
    useEffect(() => {
        ...
        const handleAudioEnd = () => {
            console.log('🏁 P21CanvasPage: Audio (18_occluder_trimmed.mp3) ended at', new Date().toISOString());
            if (audioRef.current) {
                console.log(`⏱️ P21CanvasPage: Audio actual duration: ${audioRef.current.duration.toFixed(2)}s`);
            }
    ...
    useEffect(() => {
        if (audioFinished && videoFinished && !hasAutoAdvancedRef.current) {
            ...
            console.log("🎬 P21CanvasPage: Audio and video finished, auto-advancing to next scene...");
            hasAutoAdvancedRef.current = true;
            const timer = setTimeout(() => {
                console.log("🎬 P21CanvasPage: Calling fetchNextScene to load next page...");
    ...
    useEffect(() => {
        const handleKeyPress = (e) => {
            if (e.shiftKey && (e.key === 'S' || e.key === 's')) {
                console.log("SKIP KEY PRESSED: Shift+S detected in P21CanvasPage, skipping to next page");
                e.preventDefault();
                e.stopPropagation();
                fetchNextScene(setdisableCountdownTrigger);
                return false;
    ...
    return (
        <div style={{
            ...
            <audio
                ...
                onPause={() => console.log('⏸️ P21CanvasPage: Audio paused')}
                onEnded={() => console.log('🏁 P21CanvasPage: Audio ended')}
                onError={(e) => console.error('❌ P21CanvasPage: Audio error:', e)}
            />
