 ...
 * - Canvas: T_v2_red_mid trial data (V2)
...
const P16CanvasPage = ({ fetchNextScene, setdisableCountdownTrigger }) => {
    ...
    useEffect(() => {
        if (config.startingAudioPath && config.startingAudioPath.trim() !== '') {
            ...
            audio.onloadeddata = () => {
                ...
                console.log('P16CanvasPage: Starting audio file loaded:', config.startingAudioPath);
            };
            audio.onerror = () => {
                console.warn('P16CanvasPage: Failed to load starting audio file from:', config.startingAudioPath);
    ...
    useEffect(() => {
        if (config.endingAudioPath && config.endingAudioPath.trim() !== '') {
            ...
            audio.onloadeddata = () => {
                ...
                console.log('P16CanvasPage: Ending audio file loaded:', config.endingAudioPath);
            };
            audio.onerror = () => {
                console.warn('P16CanvasPage: Failed to load ending audio file from:', config.endingAudioPath);
    ...
    // Load T_v2_red_mid trial data (V2)
    useEffect(() => {
        const loadTrialData = async () => {
            try {
                console.log('📥 P16CanvasPage: Loading T_v2_red_mid trial data from /api/load_trial_data/T_v2_red_mid...');
                const response = await fetch('/api/load_trial_data/T_v2_red_mid', {
                    method: 'GET',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    console.error('❌ P16CanvasPage: Failed to load T_v2_red_mid data:', response.status, errorText);
                }
            } catch (error) {
                ...
                if (nextFrame < maxFrames) {
                    ...
                    if (endAudioRef.current) {
                        ...
                        endAudioRef.current.play().catch(error => {
                            console.warn('P16CanvasPage: Failed to play end audio:', error);
                        });
                        console.log('🔊 P16CanvasPage: End sound played');
                    }
    ...
    useEffect(() => {
        if (isPaused) {
            console.log('⏸️ P16CanvasPage: Study paused - stopping animation and countdown');
            // Stop animation
            if (animationRef.current) {
                cancelAnimationFrame(animationRef.current);
                animationRef.current = null;
    ...
    useEffect(() => {
        ...
        if (resumeCounter > 0 && resumeCounter !== lastResumeCounterRef.current) {
            ...
            console.log('▶️ P16CanvasPage: Study resumed - resetting and restarting from beginning');
                ...
                if (countdownValue === 0) {
                    ...
                    if (startAudioRef.current) {
                        ...
                        startAudioRef.current.play().catch(error => {
                            console.warn('P16CanvasPage: Failed to play start audio:', error);
                        });
                        console.log('🔊 P16CanvasPage: Start sound played');
                    }
                }
