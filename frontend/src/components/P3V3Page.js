...
/**
 * P3V3Page - Sensor Introduction Demo
 * 
 * Audio: v3_sensors.mp3
 * Canvas: T_v3_ball_sensor_red trial data
 * Visual: Canvas with Elmo below
 * Behavior: Auto-play audio + canvas animation, auto-advance when both finish
 */
const P3V3Page = ({ onComplete }) => {
    console.log("🎬 P3V3Page: Component mounted/rendered");
    ...
    const [canvasDelayComplete, setCanvasDelayComplete] = useState(false);
    const animationRef = useRef(null);
    const lastTimestampRef = useRef(null);
    const hasAutoAdvancedRef = useRef(false);
    const renderFrameRef = useRef(null);
    const canvasDelayTimerRef = useRef(null);
    
    const CANVAS_DELAY_MS = 6000; // Start canvas 6 seconds after audio starts
    ...
    useEffect(() => {
        const loadTrialData = async () => {
            try {
                console.log('📥 P3V3Page: Loading T_v3_ball_sensor_red trial data...');
                const response = await fetch('/api/load_trial_data/T_v3_ball_sensor_red', {
                    method: 'GET',
                ...
                if (response.ok) {
                    ...
                    console.log('✅ P3V3Page: Successfully loaded trial data');
                    setSceneData(data);
                } else {
                    console.error('❌ P3V3Page: Failed to load trial data:', response.status);
                }
            } catch (error) {
                console.error('❌ P3V3Page: Error loading trial data:', error);
            }
        };
        loadTrialData();
    }, []);
    ...
    useEffect(() => {
        if (resumeCounter > 0 && resumeCounter !== lastResumeCounterRef.current) {
            ...
            console.log('▶️ P3V3Page: Resumed - restarting from beginning');
            ...
            setCanvasDelayComplete(false);
            hasAutoAdvancedRef.current = false;
            lastTimestampRef.current = null;
            
            // Clear any existing delay timer
            if (canvasDelayTimerRef.current) {
            ...
            // Render first frame immediately (frozen)
            if (renderFrameRef.current && sceneData) {
                renderFrameRef.current(0);
            }
            
            // Start canvas animation after 6 second delay
            canvasDelayTimerRef.current = setTimeout(() => {
                console.log('P3V3Page: Canvas delay complete after resume, starting animation');
                setCanvasDelayComplete(true);
                setIsPlaying(true);
            }, CANVAS_DELAY_MS);
        }
    ...
    useEffect(() => {
        ...
        // Render first frame immediately (frozen) via ref so this effect doesn't depend on renderFrame
        ...
        canvasDelayTimerRef.current = setTimeout(() => {
            console.log('P3V3Page: Canvas delay complete, starting animation');
            setCanvasDelayComplete(true);
            setIsPlaying(true);
        }, CANVAS_DELAY_MS);
    ...
    useEffect(() => {
        ...
        const handleAudioEnd = () => {
            console.log('🔊 P3V3Page: Audio finished');
