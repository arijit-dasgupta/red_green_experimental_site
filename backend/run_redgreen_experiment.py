...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PATH_TO_DATA_FOLDER = 'trial_data' # folder where all the datasets are stored (can have multiple datasets in this folder)
DATASET_NAME = 'pilot_final' # specify the name of the dataset folder
EXPERIMENT_RUN_VERSION = 'v0' # specify the name of the experiment run
# TIMEOUT_PERIOD = timedelta(hours=2)
TIMEOUT_PERIOD = timedelta(minutes=45) # specify the timeout period for the experiment
check_TIMEOUT_interval = timedelta(minutes=5) # specify the interval at which the timeout check will be performed
# check_TIMEOUT_interval = timedelta(minutes=10)
NUM_PARTICIPANTS = 60 # specify the target number of participants for the experiment
"""
Specify the additional number of participants to be considered for the experiment. The reason this is needed
is because some participants during the experiment may become inactive or have invalid responses. Therefore, we need to have a buffer to ensure that we have enough participants to complete the experiment.
"""
...
EXPERIMENTS = {
    "redgreen": {
        "major_path": f"{PATH_TO_DATA}/{DATASET_NAME}",
        data/{DATASET_NAME}",
        "num_trials": 0,
        "num_ftrials": 0,
        "trial_datas": [],
