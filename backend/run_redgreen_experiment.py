...
def load_next_scene():
    ...
    # IMPORTANT: Don't adjust ftrial_i for demonstration pages (p8=ftrial_i=1, p9=ftrial_i=2, p10=ftrial_i=3) 
    ...
    if (resume_from_trial is None and 
        ...
        # Only adjust ftrial_i if we're past the demonstration pages (ftrial_i > 3)
        # or if we have scores that indicate we've completed regular familiarization trials
        if len(fscores) < ftrial_i and ftrial_i > 3:
            # Only decrement if we're past p8/p9/p10 and there's a mismatch
            print(f"⚠️ Adjusting ftrial_i: len(fscores)={len(fscores)}, ftrial_i={ftrial_i}")
            ftrial_i -= 1
        elif len(fscores) < ftrial_i and ftrial_i <= 3:
            # For p8 (ftrial_i=1), p9 (ftrial_i=2), and p10 (ftrial_i=3), don't adjust - they're demonstration pages
