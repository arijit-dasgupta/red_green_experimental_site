...
def load_next_scene():
    ...
    # IMPORTANT: Don't adjust ftrial_i for demonstration/practice pages (p8=ftrial_i=1, p9=ftrial_i=2, p10=ftrial_i=3, p11=ftrial_i=4, p12=ftrial_i=5, p14=ftrial_i=6, p15=ftrial_i=7) 
    ...
    if (resume_from_trial is None and 
        ...
        # Only adjust ftrial_i if we're past the demonstration/practice pages (ftrial_i > 7)
        # or if we have scores that indicate we've completed regular familiarization trials
        if len(fscores) < ftrial_i and ftrial_i > 7:
            # Only decrement if we're past p8/p9/p10/p11/p12/p14/p15 and there's a mismatch
            print(f"⚠️ Adjusting ftrial_i: len(fscores)={len(fscores)}, ftrial_i={ftrial_i}")
            ftrial_i -= 1
        elif len(fscores) < ftrial_i and ftrial_i <= 7:
            # For p8 (ftrial_i=1), p9 (ftrial_i=2), p10 (ftrial_i=3), p11 (ftrial_i=4), p12 (ftrial_i=5), p14 (ftrial_i=6), and p15 (ftrial_i=7), don't adjust - they're demonstration/practice pages
